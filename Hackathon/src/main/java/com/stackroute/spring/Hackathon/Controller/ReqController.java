...
import com.stackroute.spring.Hackathon.Exception.NotValidException;
...
public class ReqController {
	...
	public ResponseEntity<String> add_user(@RequestBody UserModel user) throws NotValidException{
		if(user.getName()==null || user.getEmail()==null) {
			throw new NullPointerException("Invalid field");		}
		if(!(user.getEmail().matches("^([a-zA-Z0-9_\\-\\.]+)@([a-zA-Z0-9_\\-\\.]+)\\.([a-zA-Z]{2,5})$")))
	...
	public @ResponseBody Iterable<UserModel> getAllUser(){
		...
		Iterable<UserModel> user=service.allUser();
